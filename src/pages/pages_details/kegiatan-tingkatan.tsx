...
const KegiatanTingkatan = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response: ActivitiesResponse = await getRequest(
    ...
    <>
      <div className="kt-page">
        ...
        <div className="kt-breadcrumb" data-aos="fade-down">
          ...
          <span className="kt-breadcrumb-current">
        ...
        <div className="kt-wrapper">
          ...
          <div className="kt-header" data-aos="fade-down">
            <div
              className="kt-icon"
              style={{ background: getLevelColor(params.level) }}
            >
              {getLevelIcon(params.level)}
            </div>
            <h1 className="kt-title">Kegiatan {getLevelTitle(params.level)}</h1>
            <p className="kt-desc">
              Menampilkan {data && data.length} kegiatan untuk tingkat{" "}
              {getLevelTitle(params.level)}
            </p>
          </div>
          ...
          {data && data.length > 0 ? (
                  ...
                  data.map((act, index) => (
                    ...
                    >
                      ...
                      <div className="kt-content">
                        <span className="kt-date">
                          📅 {formatTime(act.tanggal_publikasi, "DD MMMM yyyy")}
                        </span>
                        <h2>{act.judul}</h2>
                        <p>{act.konten}</p>
                        <button
                          className="kt-btn"
                          onClick={() =>
                            navigate(`/kegiatan/${act.kegiatan_id}`)
                          }
                        >
            ...
            <div className="kt-empty" data-aos="fade-up">
              ...
              <p>
                Belum ada kegiatan untuk tingkat {getLevelTitle(params.level)}{" "}
                saat ini.
              </p>
