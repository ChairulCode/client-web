...
const PengumumanTingkatan = () => {
  ...
  const [backupData, setBackupData] = useState<
    AnnouncementsResponse["data"] | null
  >(null);
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        const response: AnnouncementsResponse = await getRequest(
          "pengumuman?page=1&limit=1000",
        );
                    ...
                    <div className="pt-card-footer">
                      {announcement.jenjang_relasi.map((level) => (
                        <span className="pt-card-level">
                          {getLevelTitle(level.jenjang.nama_jenjang)}
                        </span>
                      ))}
                    </div>
